  ...
  var loadMyCartEvent = function (targetSelector) {
    ...
    if (!$("#" + idCartModal).length) {
      $('body').append(
        ...
        '<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"><i class="fa-duotone fa-square-xmark"></i></button>' +
        '<h5 class="modal-title" id="myModalLabel"><i class="fa-duotone fa-cart-shopping"></i> سلة المشتريات</h5>' +
        '</div>' +
        '<div class="modal-body">' +
        '<table class="table table-hover table-responsive" id="' + idCartTable + '"></table>' +
        '</div>' +
        '<div class="modal-footer">' +
        '<button type="button" class="btn rounded-0 btn-secondary" data-bs-dismiss="modal">استكمال التسوق</button>' +
        '<button type="button" class="btn azab-btn btn-primary ' + classCheckoutCart + '">إتمام الشراء</button>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</div>'
    ...
    var drawTable = function () {
      ...
      $cartTable.append(products.length ?
        ...
        '<td></td>' +
        '</tr>' :
        '<div class="alert alert-danger" role="alert" id="' + idEmptyCartMessage + '">لا توجد منتجات في السلة</div>'
      );
